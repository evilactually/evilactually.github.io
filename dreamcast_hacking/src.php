...
<p>    
    <?img("Dreamcast_logo.svg", 20, "float:right;margin-bottom: 1ch; margin-left: 2ch;");?>
  </picture>
  ...
  <?h("First protection level: GD-ROM");?><p style="margin-bottom: 0;">
   ...
   <picture  >
    ...
    <?img("ikaruga_gdrom.png", 21, "float:left;margin-bottom: 2ch; margin-left: 2ch; margin-right:1ch;");?>
  </picture>
    <?img("gd-rom.svg", 21, "float:left;margin-bottom: 2ch; margin-right: 2ch;");?>
   ...
   <br/> The first (dark) section was a low density, CD-ROM compatible, area holding up to 35 MiB. It contained a voiceover audio track reminding user that the content was intended for SEGA Dreamcast and not for a CD player<?footnote("SEGA GD Workshop", "https://www.consolecopyworld.com/dc/gd-ws/gdws_big/sld009.htm");?>.
...
<p>
    The (clear) high density section storing up to 984 MiB<?footnote("segaretro.org: GD-ROM", "https://segaretro.org/GD-ROM");?> is where all the game's content was located.
 ...
 <?h("Booting from a GD-ROM: IP.BIN and 1ST_READ.BIN");?><p>
    ...
    More importantly, IP.BIN contained the name of the game executable. This name was looked up in the GD-ROM file-system and loaded in RAM at 0x8C010000 before execution was transfered there. Usually the filename of the game executable was "1ST_READ.BIN".<br/>
       <br/> 
    <?img("gd-rom_loading.svg", 100, "margin-top: 2ch; margin-bottom: 2ch;");?>
   <br/>
 ...
 <?h("Second protection level: Scrambler");?><p> 
  ...
  <br/>
    ...
    <?img("cd-rom_loading.svg", 100, "margin-top: 2ch; margin-bottom: 2ch;");?>
   ...
   <?h("Give me my long sword, ho!");?><p>
    The mashed potatoes problem was solved when a Katana SDK (the official Sega SDK for the Dreamcast) was stolen<?footnote("Let's build a Sega Dreamcast game from scratch", "https://www.neogaf.com/threads/lets-build-a-sega-dreamcast-game-from-scratch-breakout.916501/page-3#post-216433495");?> by the hacking team "Utopia" in late 1999. It turned out that the scrambler was nothing more than "security through obscurity". The SDK contained a reverse-scrambler which transformed a valid executable into reverse-mashed-potatoes so it would be valid again when loaded and scrambled by the Dreamcast when booting from a CD-ROM.<br/>
    <?img("cd-rom_loading_reverse.svg", 100, "margin-top: 2ch; margin-bottom: 2ch;");?>
   <br/>
 </p>
  <div style="clear:both;"></div>
 ...
 <?h("Extracting a game from its GD-ROM");?><p>
  The stolen SDK was all pirates needed. With the ability to run code on the machine, the Dreamcast was re-purposed to act not as a game console but as a GD-ROM drive. The SDK's "Coder's Cable"<?footnote("PC Serial Adaptor", "http://mc.pp.se/dc/serifc.html");?> allowed to connect the console to a PC and establish a physical connection. To trigger the console to dump the GD track content, a special executable was written, reverse-scrambled and burned onto a CD-ROM in order to output the whole 1 GiB via the console's serial port. It was an error prone process which took up to 18 hours<?footnote("A more accurate and in-depth look at the Dreamcast's security", "https://www.reddit.com/r/dreamcast/comments/15pww3/a_more_accurate_and_indepth_look_at_the/");?> to complete<?footnote("Faster ways were designed latter, using the DC's broadband connector", "https://assemblergames.com/threads/dumping-dreamcast-gd-rom-gd-r-discs.16679/");?>. The result was stored in a custom made format called ".gdi".<pre>ikaruga.gdi            153 bytes
track01.bin     13,982,640 bytes
track02.raw      2,088,576 bytes
track03.bin  1,185,760,800 bytes</pre>
<p>
<b>Trivia</b>: You will notice that the total account for 1.2 GiB and not the 1.0 GiB mentioned earlier. This is because GD-ROM 2,352-byte sectors follow the "Red Book" format where 12 bytes are used for synchronization, 4 bytes for the header, 2048 bytes for the payload, and 288 bytes are for Error Detection Code/Error Correcting Code<?footnote("Dreamcast Myth: GD-ROM Storage Capacity", "https://xiaopang333.wordpress.com/2008/08/06/dreamcast-myth-gd-rom-storage-capacity/");?>.<br/>
<pre>$ cat ikaruga.gdi 
3
...
3 45000 4 2352 track03.bin 0</pre>
 ...
 <?h("Making a 1000 MiB GD-ROM fit inside a 700 MiB CD-ROM");?><p>
...
The last two steps of the process were to reverse-scramble 1ST_READ.BIN and pack everything into a .cdi archive so <a href="https://legacy.padus.com/">DiscJuggler</a> could burn the image on a CD-R. The result ran flawlessly on any vanilla Dreamcast without need for a modification chip.<br/>
 ...
 <?h("Response from SEGA and after-match");?><p>
